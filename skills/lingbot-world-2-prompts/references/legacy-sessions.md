# Legacy sessions (verbatim from LingBot-World 2 export data)

**These sessions predate the layered-scene format** the skill now authors
(see SKILL.md): each is one monolithic base prompt with the camera and
motion contract baked in, plus per-key addenda appended after the base,
exported as `actions.json` with `f`/`g`/numbered/`space` slots. Two reasons
to read this file:

1. **A user asks for the legacy export format.** The shape: the base prompt
   in `prompt.txt`; `actions.json` with an `actions` array holding one entry
   per key in order `f, g, 1..N, space`, each entry carrying `id`
   (`<slot>#<cand_index>`), `slot`, `cand_index` (0 unless multiple
   candidates), `label_en`, `addendum_en`, `prompt_en` = base + single
   space + addendum verbatim (never paraphrased), `_zh` fields duplicating
   `_en` unless the user supplies Chinese, and `selected: false`. `f` was a
   contextual scene event (with the subject-preservation clause), `g` an
   environment transformation, `space` the fixed string "The current
   controllable subject springs upward into the air." with label `Jump`.
   Session 1 shows the older fixed `u` (melee) / `o` (ranged) combat slots.
2. **Event-shape inspiration.** The addendum craft here — chained
   encounters that resolve the previous key before introducing the next,
   teleport tours with named transition devices, toggle pairs, and subject
   transformations that settle into a new stable state — carries over
   verbatim into layered-scene `events`.
...
session 1 is an earlier June export kept to show the playable-character base
template and the legacy `u`/`o` layout.
